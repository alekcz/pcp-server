...
The `home-routes` handler in the `pcp.routes.home` namespace
...
The `home-page` function will in turn call the `pcp.layout/render` function
...
The routes are aggregated and wrapped with middleware in the `pcp.handler` namespace:
...
Request middleware functions are located under the `pcp.middleware` namespace.

This namespace is reserved for any custom middleware for the application. Some default middleware is
already defined here. The middleware is assembled in the `wrap-base` function.

Middleware used for development is placed in the `pcp.dev-middleware` namespace found in
